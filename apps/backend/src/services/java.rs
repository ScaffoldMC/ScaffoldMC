...
impl JavaService {
	...
	pub async fn get_suitable_for(
		&self,
		major_version: u8,
	) -> Result<JavaVersion, JavaServiceError> {
		let javas = self.get_versions().await?;
		let suitable = javas
			.into_iter()
	...
	pub async fn get_versions(&self) -> Result<Vec<JavaVersion>, JavaServiceError> {
		let jvm_paths: Vec<PathBuf> = {
			...
			#[cfg(target_os = "macos")]
			{
				...
				if !java_dir.exists() && !java_dir.is_dir() {
					return Err(JavaServiceError::FileSystem(
