...
impl JavaService {
	...
	pub fn get_suitable_for(&self, major_version: u8) -> Result<JavaVersion, JavaError> {
		let javas = self.get_versions()?;
		let suitable = javas
			.into_iter()
	...
	pub fn get_versions(&self) -> Result<Vec<JavaVersion>, JavaError> {
